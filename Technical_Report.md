...
- **Embedding Model**: The application utilizes `BAAI/bge-small-en` (configurable via `.env`) and supports localized models via the LLMFactory pattern. This choice is justified due to its balance between processing speed, low memory footprint, and high accuracy for semantic representation on consumer-grade hardware. For multi-lingual and Arabic contexts, replacing this with a multilingual variant integrates seamlessly into the `local_bge` provider.
